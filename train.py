...
sys.path.append(r"../")
sys.path.append(r"../../")
from model.sbrl_hapi import sbrl_hapi
from utils import *
from model.IvR import run as run_IvR
...
tf.app.flags.DEFINE_string('outdir', '/ossfs/workspace/SBRL-HAPI/results/syn_i8_c8_a8_d2_n10000_biny/rp25_train/res3', """Output directory. """)
tf.app.flags.DEFINE_string('datadir', '/ossfs/workspace/SBRL-HAPI/data/Syn_8_8_8_2_10000/syn_biny/', """Data directory. """)
...
def run(net, sess, i_exp, D_exp, D_exp_test, I_valid, logfile):
    ...
    train_dict = {net.x: D_exp['x'], net.s: D_exp['s'], net.t: D_exp['t'], net.y_: D_exp['yf'], \
                  net.do_in: 1.0, net.do_out: 1.0, net.p_t: p_treated, net.idx: I, net.n: n}
    if D_exp['ycf'] is not None:
        train_cf_dict = {net.x: D_exp['x'], net.s: D_exp['s'], net.t: D_exp['t'], net.ycf: D_exp['ycf'], \
                         net.do_in: 1.0, net.do_out: 1.0, net.p_t: p_treated, net.idx: I, net.n: n}
    ...
    for i in range(FLAGS.iterations):
        ...
        s_batch = D_exp['s'][I_train,:][I_batch]
        t_batch = D_exp['t'][I_train,:][I_batch]
        y_batch = D_exp['yf'][I_train,:][I_batch]

        batch_dict = {net.x: x_batch, net.s: s_batch, net.t: t_batch, net.y_: y_batch, \
        ...
        if not objnan:
            ...
            if (FLAGS.reweight_sample or FLAGS.p_alpha>0) and not w_objnan:
                batch_cov_dict = {net.x: x_batch, net.s: s_batch, net.t: t_batch, net.y_: y_batch, net.do_in: 1.0, net.do_out: 1.0, net.p_t: p_treated, net.idx: I_batch, net.n:len(I_batch)}
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            for dt_i in range(len(D_exp_test)):
                test_dict = {net.x: D_exp_test[dt_i]['x'], net.s: D_exp_test[dt_i]['s'], net.t: D_exp_test[dt_i]['t'], net.y_: D_exp_test[dt_i]['yf'], \
    ...
    for di in range(len(D_exp_test)):
        pred_hidden = sess.run(net.mus_Y, feed_dict={net.x: D_exp_test[di]['x'], net.s: D_exp_test[di]['s'], net.t: D_exp_test[di]['t'], net.do_in: 1.0, net.do_out: 1.0})
    ...
    rep_hidden, weight_in, pred_hidden, weight_pred = sess.run([net.rep_C, net.w_C, net.mus_Y, net.w_muY], \
                                                               feed_dict={net.x: D_exp['x'], net.s: D_exp['s'], net.t: D_exp['t'], net.do_in: 1.0, net.do_out: 1.0})
...
def train(outdir):
    ...
    for i_exp in range(1,FLAGS.n_experiments+1):
        ...
        for dt_i in range(len(D_test)):
            ...
            D_exp_test['v']  = D_test[dt_i]['v'][:,:,(i_exp-1)%n_exp_test]
        ...
        D_exp, D_exp_test_list = run_IvR(exp=i_exp-1, resultDir=outdir,train_dict=D_exp, test_ood_dict=D_exp_test_list[0], ood_test_dict=D_exp_test_list)
