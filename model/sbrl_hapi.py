...
class sbrl_hapi(object):
    """
    This file contains the class for SBRL-HAPI.
    ...
    by Wenyang Liu and Yuling Zhang et.al
    """
    ...
    def __init__(self, FLAGS, dims):
        ...
        self.s = tf.placeholder("float", shape=[None, 1], name='s')   # Saved Propensity Score
    ...
    def build_graph(self, dims):
        ...
        with tf.variable_scope('outcome'):
            self.mu_YF, self.mu_YCF, self.mu_Y_0, self.mu_Y_1, self.mus_Y, self.w_muY, self.b_muY = self.output(data_input=self.rep_C,
                                                                                                    dim_in=self.FLAGS.rep_dim,
                                                                                                    dim_out=self.FLAGS.y_dim,
                                                                                                    layer=self.FLAGS.y_layer,
                                                                                                    name='Mu_ytx')
        ''' iv regression '''
        with tf.variable_scope('ivreg'):
            if self.FLAGS.twoStage:
                self.mu_YF = self.s * self.mu_Y_1 + (1 - self.s) * self.mu_Y_0
    ...
    def output(self, data_input, dim_in, dim_out, layer, name, mode='mu'):
        ...
        return mu_YF, mu_YCF, mu_Y_0, mu_Y_1, mus_Y, w_muY, b_muY
